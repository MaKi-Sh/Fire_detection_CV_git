...
MODELS = {
    "yolo11n": {
        "weights": "yolo11n.pt",
        "project_name": "Fire-detect_yolo11n_recall_opt",
    },
    "yolo11s": {
        "weights": "yolo11s.pt",
        "project_name": "Fire-detect_yolo11s_recall_opt",
    },
    "yolo11m": {
        "weights": "yolo11m.pt",
        "project_name": "Fire-detect_yolo11m_recall_opt",
    },
}
...
def make_train_fn(model_key, best_tracker):
    ...
    def train():
        ...
        results = model.train(
            ...
            name=f'sweep_{model_key}_recall_{wandb.run.id}',
            epochs=100,
            patience=20,
            device=0,
        ...
        try:
            if hasattr(results, 'save_dir'):
                ...
                save_dir = f'/home/makish/Desktop/Science_fair_2025/Fire_detection_CV_git/runs/sweep_{model_key}_recall_{wandb.run.id if wandb.run else "unknown"}'
        ...
        # Track best run by overall recall
        try:
            if hasattr(results, 'results_dict'):
                recall_val = results.results_dict.get('metrics/recall(B)', -1)
                if recall_val > best_tracker.get("best_metric", -1):
                    best_tracker["best_metric"] = recall_val
...
if __name__ == "__main__":
    ...
    for model_key, model_info in MODELS.items():
        print(f"\n{'#'*60}")
        print(f"# Starting sweep for {model_key} (maximizing recall)")
        ...
        if "save_dir" in best_tracker:
            ...
            print(f"  Best recall: {best_tracker['best_metric']:.4f}")
        else:
